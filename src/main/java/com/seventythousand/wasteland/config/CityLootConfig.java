...
public class CityLootConfig {
  ...
  public static void load(Configuration config) {
    ...
    easyLootRepeat = config.getBoolean("Common city chest item duplicates", "Loot Amount", true, "Set to false to prevent item duplicates in a spawned chest. If items MAX > total types of items, item MAX = total types");
    ...
    midLootRepeat = config.getBoolean("Uncommon city chest item duplicates", "Loot Amount", true, "Set to false to prevent item duplicates in a spawned chest. If items MAX > total types of items, item MAX = total types");
    ...
    hardLootRepeat = config.getBoolean("Rare city chest item duplicates", "Loot Amount", false, "Set to false to prevent item duplicates in a spawned chest. If items MAX > total types of items, item MAX = total types");
    ultraLootMax = config.getInt("Ultra city chest items MAX", "Loot Amount", 16, 0, 27, "");
    ultraLootMin = config.getInt("Ultra city chest items MIN", "Loot Amount", 8, 0, 27, "");
    ultraLootRepeat = config.getBoolean("Rare city chest item duplicates", "Loot Amount", false, "Set to false to prevent item duplicates in a spawned chest. If items MAX > total types of items, item MAX = total types");
    ...
    midLoot = config.get("Loot List", "Uncommon chest loot items", new String[] { "minecraft:bucket,2,1,3", "hbm:item.canister_empty,3,2,5", "minecraft:potion,4,3,4", "hbm:item.horseshoe_magnet,1,1,2", "minecraft:paper,20,10,6", "hbm:item.pill_herbal,2,1,4", "hbm:tile.deco_crt,3,2,5", "hbm:item.coffee,3,1,5", "hbm:tile.radiorec,2,1,4", "hbm:item.cigarette,6,2,6", "hbm:item.screwdriver,1,1,5", "hbm:item.rag,6,2,4", "hbm:item.flame_conspiracy,5,2,4", "hbm:item.flame_politics,5,2,4", "hbm:item.plant_item:1,6,2,4", "hbm:item.radx,3,1,4", "hbm:item.lodestone,1,1,3" }).getStringList();
    hardLoot = config.get("Loot List", "Rare chest loot items", new String[] { "hbm:tile.radiorec,3,2,6",  "hbm:item.jackt,1,1,2", "hbm:item.syringe_metal_stimpak,6,4,7", "minecraft:diamond,2,1,6", "hbm:item.siphon,1,1,2", "hbm:item.blowtorch,1,1,2", "hbm:item.boltgun,1,1,1", "hbm:item.defuser,1,1,2", "hbm:item.weapon_saw,1,1,2", "hbm:item.bolt:30,24,12,6", "hbm:item.bolt:33,24,12,6", "hbm:item.canister_full:20,4,1,6", "hbm:item.canister_full:20,4,1,6", "hbm:tile.deco_crt,3,1,4",  "hbm:item.serum,1,1,3",  "hbm:item.spider_milk,1,1,3",  "hbm:tile.deco_computer,3,1,3",  "hbm:item.weapon_golf_club,1,1,3",  "hbm:item.med_bag,1,1,5",  "hbm:item.siox,1,1,4", "hbm:item.bandaid,1,1,4", "hbm:item.armor_polish,1,1,2"}).getStringList();
    ultraLoot = config.get("Loot List", "Ultra rare chest loot items", new String[] { "hbm:item.cheese,14,12,6", "hbm:item.loops,10,2,6", "hbm:item.scrumpy,1,1,3", "hbm:item.gun_heavy_revolver,1,1,2", "hbm:item.wd40,1,1,6", "hbm:item.gun_uzi,1,1,1", "hbm:item.bathwater,1,1,3", "hbm:item.acetylene_torch,1,1,3",  "hbm:item.ammo_standard:22,16,8,8", "hbm:item.ammo_standard:12,16,8,8", "hbm:tile.deco_toaster,1,1,8", "hbm:item.siox,3,2,6", "hbm:item.radx,12,8,8", "hbm:item.syringe_metal_medx,16,8,8",  "hbm:item.morning_glory,1,1,3", "hbm:item.syringe_metal_stimpak,12,8,6", "hbm:item.syringe_metal_psycho,12,8,6","hbm:tile.deco_crt,4,2,4", "hbm:item.heart_piece,1,1,6", "hbm:item.shimmer_head,1,1,1", "hbm:item.shimmer_axe_head,1,1,1", "hbm:item.shimmer_handle,1,1,3" }).getStringList();
